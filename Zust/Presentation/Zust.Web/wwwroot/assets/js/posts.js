function getPosts() {
    var skip = $(".news-feed-area>.news-feed-post").length
    ...
    if(controller == "Home" || controller==""){
        url = "/Post/GetNewsFeedPosts?skip="+skip;
    }
    else{
        username = $("#profilphoto").attr("value");
        url = "/Post/GetUserPosts?username="+username+"&skip="+skip;
    }
    $.ajax({
        type: "POST",
        url: url,
        success: function (data) {
            $.each(data, function (index, item) {
                    ...
                    <div class="news-feed news-feed-post">
                                        <div class="post-header d-flex justify-content-between align-items-center">
                                            ...
                                            `<div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li><a class="dropdown-item d-flex align-items-center" href="/Post/DeletePost/${item.postId}"><i class="flaticon-trash"></i> Delete Post</a></li>
                                                </ul>
                                            </div>` : ""
                                            }
                                        </div>
                                        ...
                                        <div class="post-body">
                                            ...
                                            <div class="post-image">
                                                ...
                                                <button class="bg-transparent border-0" type="button" data-bs-toggle="modal" id="modal${item.postId}" data-bs-target="#modaltriger${item.postId}">
                                                    <i class="fa-solid fa-user-tag"></i>
                                                </button>

                                                <!-- Modal -->
                                                <div class="modal fade" id="modaltriger${item.postId}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                if(item.postTagsUsernames.length <=0){
                    $("#modal"+item.postId).remove();
                    $("#modaltriger"+item.postId).remove();
                }
                else{
                    var modal = $("#modaltriger"+item.postId).find(".modal-body")
    
    
                    $.each(item.postTagsUsernames,function(index,username){
                        debugger
                        modal.append(`<span class="d-block">${username}</span>`)
                    })
                }
            })
